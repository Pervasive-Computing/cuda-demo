...
double* generate_gaussian_kernel(int N) {
    ...
    for (int y = -offset; y <= offset; y++) {
        for (int x = -offset; x <= offset; x++) {
            ...
            kernel[index] = (1.0 / (2 * M_PI * SIGMA * SIGMA)) * std::exp(-(x * x + y * y) / (2 * SIGMA * SIGMA));
            sum += kernel[index];
        }
    }
...
auto main(int argc, char **argv) -> int {
  ...
  stbi_write_png(output_filename.c_str(), width, height, channels, output_data, width * channels);
